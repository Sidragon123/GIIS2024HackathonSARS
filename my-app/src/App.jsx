import "./App.css";
import background from "./AppAssets/background.mp4";
import "bootstrap/dist/css/bootstrap.css";
import {
  Card,
  CardSection,
  Container,
  HoverCard,
  HoverCardDropdown,
  HoverCardTarget,
  MantineContext,
  MantineProvider,
  ScrollArea,
  Group,
  Stack,
  Text,
} from "@mantine/core";
import { useState } from "react";

// var requestOptions = {
//   method: "GET",
//   redirect: "follow",
// };
// const fullElementList = async () => {
//   const allElementsRaw = await fetch(
//     "https://periodic-table-elements-info.herokuapp.com/elements",
//     requestOptions
//   );
//   const allElements = await allElementsRaw.json();
//   console.log(allElements);
//   console.log(allElements[1]);
// };
// fullElementList();
const elementData = [
  {},
  {
    atomicNumber: 1,
    symbol: "H",
    name: "Hydrogen",
    atomicMass: "1.00794(4)",
    electronicConfiguration: "1s1",
    electronegativity: 2.2,
    atomicRadius: 37,
    ionRadius: "unknown",
    vanDerWaalsRadius: 120,
    ionizationEnergy: 1312,
    electronAffinity: -73,
    oxidationStates: "-1, 1",
    standardState: "gas",
    bondingType: "diatomic",
    meltingPoint: 14,
    boilingPoint: 20,
    density: 0.0000899,
    groupBlock: "nonmetal",
    yearDiscovered: 1766,
    block: "s",
    cpkHexColor: "FFFFFF",
    period: 1,
    group: 1,
  },
  {
    atomicNumber: 2,
    symbol: "He",
    name: "Helium",
    atomicMass: "4.002602(2)",
    electronicConfiguration: "1s2",
    electronegativity: "unknown",
    atomicRadius: 32,
    ionRadius: "unknown",
    vanDerWaalsRadius: 140,
    ionizationEnergy: 2372,
    electronAffinity: 0,
    oxidationStates: "unknown",
    standardState: "gas",
    bondingType: "atomic",
    meltingPoint: "unknown",
    boilingPoint: 4,
    density: 0.0001785,
    groupBlock: "noble gas",
    yearDiscovered: 1868,
    block: "p",
    cpkHexColor: "D9FFFF",
    period: 1,
    group: 18,
  },
  {
    atomicNumber: 3,
    symbol: "Li",
    name: "Lithium",
    atomicMass: "6.941(2)",
    electronicConfiguration: "[He] 2s1",
    electronegativity: 0.98,
    atomicRadius: 134,
    ionRadius: "76 (+1)",
    vanDerWaalsRadius: 182,
    ionizationEnergy: 520,
    electronAffinity: -60,
    oxidationStates: 1,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 454,
    boilingPoint: 1615,
    density: 0.535,
    groupBlock: "alkali metal",
    yearDiscovered: 1817,
    block: "s",
    cpkHexColor: "CC80FF",
    period: 2,
    group: 1,
  },
  {
    atomicNumber: 4,
    symbol: "Be",
    name: "Beryllium",
    atomicMass: "9.012182(3)",
    electronicConfiguration: "[He] 2s2",
    electronegativity: 1.57,
    atomicRadius: 90,
    ionRadius: "45 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 900,
    electronAffinity: 0,
    oxidationStates: 2,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1560,
    boilingPoint: 2743,
    density: 1.848,
    groupBlock: "alkaline earth metal",
    yearDiscovered: 1798,
    block: "s",
    cpkHexColor: "C2FF00",
    period: 2,
    group: 2,
  },
  {
    atomicNumber: 5,
    symbol: "B",
    name: "Boron",
    atomicMass: "10.811(7)",
    electronicConfiguration: "[He] 2s2 2p1",
    electronegativity: 2.04,
    atomicRadius: 82,
    ionRadius: "27 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 801,
    electronAffinity: -27,
    oxidationStates: "1, 2, 3",
    standardState: "solid",
    bondingType: "covalent network",
    meltingPoint: 2348,
    boilingPoint: 4273,
    density: 2.46,
    groupBlock: "metalloid",
    yearDiscovered: 1807,
    block: "p",
    cpkHexColor: "FFB5B5",
    period: 1,
    group: 13,
  },
  {
    atomicNumber: 6,
    symbol: "C",
    name: "Carbon",
    atomicMass: "12.0107(8)",
    electronicConfiguration: "[He] 2s2 2p2",
    electronegativity: 2.55,
    atomicRadius: 77,
    ionRadius: "16 (+4)",
    vanDerWaalsRadius: 170,
    ionizationEnergy: 1087,
    electronAffinity: -154,
    oxidationStates: "-4, -3, -2, -1, 1, 2, 3, 4",
    standardState: "solid",
    bondingType: "covalent network",
    meltingPoint: 3823,
    boilingPoint: 4300,
    density: 2.26,
    groupBlock: "nonmetal",
    yearDiscovered: "Ancient",
    block: "p",
    cpkHexColor: "909090",
    period: 2,
    group: 14,
  },
  {
    atomicNumber: 7,
    symbol: "N",
    name: "Nitrogen",
    atomicMass: "14.0067(2)",
    electronicConfiguration: "[He] 2s2 2p3",
    electronegativity: 3.04,
    atomicRadius: 75,
    ionRadius: "146 (-3)",
    vanDerWaalsRadius: 155,
    ionizationEnergy: 1402,
    electronAffinity: -7,
    oxidationStates: "-3, -2, -1, 1, 2, 3, 4, 5",
    standardState: "gas",
    bondingType: "diatomic",
    meltingPoint: 63,
    boilingPoint: 77,
    density: 0.001251,
    groupBlock: "nonmetal",
    yearDiscovered: 1772,
    block: "p",
    cpkHexColor: "3050F8",
    period: 2,
    group: 15,
  },
  {
    atomicNumber: 8,
    symbol: "O",
    name: "Oxygen",
    atomicMass: "15.9994(3)",
    electronicConfiguration: "[He] 2s2 2p4",
    electronegativity: 3.44,
    atomicRadius: 73,
    ionRadius: "140 (-2)",
    vanDerWaalsRadius: 152,
    ionizationEnergy: 1314,
    electronAffinity: -141,
    oxidationStates: "-2, -1, 1, 2",
    standardState: "gas",
    bondingType: "diatomic",
    meltingPoint: 55,
    boilingPoint: 90,
    density: 0.001429,
    groupBlock: "nonmetal",
    yearDiscovered: 1774,
    block: "p",
    cpkHexColor: "FF0D0D",
    period: 2,
    group: 16,
  },
  {
    atomicNumber: 9,
    symbol: "F",
    name: "Fluorine",
    atomicMass: "18.9984032(5)",
    electronicConfiguration: "[He] 2s2 2p5",
    electronegativity: 3.98,
    atomicRadius: 71,
    ionRadius: "133 (-1)",
    vanDerWaalsRadius: 147,
    ionizationEnergy: 1681,
    electronAffinity: -328,
    oxidationStates: -1,
    standardState: "gas",
    bondingType: "atomic",
    meltingPoint: 54,
    boilingPoint: 85,
    density: 0.001696,
    groupBlock: "halogen",
    yearDiscovered: 1670,
    block: "p",
    cpkHexColor: "9eaf51",
    period: 2,
    group: 17,
  },
  {
    atomicNumber: 10,
    symbol: "Ne",
    name: "Neon",
    atomicMass: "20.1797(6)",
    electronicConfiguration: "[He] 2s2 2p6",
    electronegativity: "unknown",
    atomicRadius: 69,
    ionRadius: "unknown",
    vanDerWaalsRadius: 154,
    ionizationEnergy: 2081,
    electronAffinity: 0,
    oxidationStates: "unknown",
    standardState: "gas",
    bondingType: "atomic",
    meltingPoint: 25,
    boilingPoint: 27,
    density: 0.0009,
    groupBlock: "noble gas",
    yearDiscovered: 1898,
    block: "p",
    cpkHexColor: "B3E3F5",
    period: 2,
    group: 18,
  },
  {
    atomicNumber: 11,
    symbol: "Na",
    name: "Sodium",
    atomicMass: "22.98976928(2)",
    electronicConfiguration: "[Ne] 3s1",
    electronegativity: 0.93,
    atomicRadius: 154,
    ionRadius: "102 (+1)",
    vanDerWaalsRadius: 227,
    ionizationEnergy: 496,
    electronAffinity: -53,
    oxidationStates: "-1, 1",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 371,
    boilingPoint: 1156,
    density: 0.968,
    groupBlock: "alkali metal",
    yearDiscovered: 1807,
    block: "s",
    cpkHexColor: "AB5CF2",
    period: 3,
    group: 1,
  },
  {
    atomicNumber: 12,
    symbol: "Mg",
    name: "Magnesium",
    atomicMass: "24.3050(6)",
    electronicConfiguration: "[Ne] 3s2",
    electronegativity: 1.31,
    atomicRadius: 130,
    ionRadius: "72 (+2)",
    vanDerWaalsRadius: 173,
    ionizationEnergy: 738,
    electronAffinity: 0,
    oxidationStates: "1, 2",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 923,
    boilingPoint: 1363,
    density: 1.738,
    groupBlock: "alkaline earth metal",
    yearDiscovered: 1808,
    block: "s",
    cpkHexColor: "8AFF00",
    period: 3,
    group: 2,
  },
  {
    atomicNumber: 13,
    symbol: "Al",
    name: "Aluminum",
    atomicMass: "26.9815386(8)",
    electronicConfiguration: "[Ne] 3s2 3p1",
    electronegativity: 1.61,
    atomicRadius: 118,
    ionRadius: "53.5 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 578,
    electronAffinity: -43,
    oxidationStates: "1, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 933,
    boilingPoint: 2792,
    density: 2.7,
    groupBlock: "metal",
    yearDiscovered: "Ancient",
    block: "p",
    cpkHexColor: "BFA6A6",
    period: 3,
    group: 13,
  },
  {
    atomicNumber: 14,
    symbol: "Si",
    name: "Silicon",
    atomicMass: "28.0855(3)",
    electronicConfiguration: "[Ne] 3s2 3p2",
    electronegativity: 1.9,
    atomicRadius: 111,
    ionRadius: "40 (+4)",
    vanDerWaalsRadius: 210,
    ionizationEnergy: 787,
    electronAffinity: -134,
    oxidationStates: "-4, -3, -2, -1, 1, 2, 3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1687,
    boilingPoint: 3173,
    density: 2.33,
    groupBlock: "metalloid",
    yearDiscovered: 1854,
    block: "p",
    cpkHexColor: "F0C8A0",
    period: 3,
    group: 14,
  },
  {
    atomicNumber: 15,
    symbol: "P",
    name: "Phosphorus",
    atomicMass: "30.973762(2)",
    electronicConfiguration: "[Ne] 3s2 3p3",
    electronegativity: 2.19,
    atomicRadius: 106,
    ionRadius: "44 (+3)",
    vanDerWaalsRadius: 180,
    ionizationEnergy: 1012,
    electronAffinity: -72,
    oxidationStates: "-3, -2, -1, 1, 2, 3, 4, 5",
    standardState: "solid",
    bondingType: "covalent network",
    meltingPoint: 317,
    boilingPoint: 554,
    density: 1.823,
    groupBlock: "nonmetal",
    yearDiscovered: 1669,
    block: "p",
    cpkHexColor: "FF8000",
    period: 3,
    group: 15,
  },
  {
    atomicNumber: 16,
    symbol: "S",
    name: "Sulfur",
    atomicMass: "32.065(5)",
    electronicConfiguration: "[Ne] 3s2 3p4",
    electronegativity: 2.58,
    atomicRadius: 102,
    ionRadius: "184 (-2)",
    vanDerWaalsRadius: 180,
    ionizationEnergy: 1000,
    electronAffinity: -200,
    oxidationStates: "-2, -1, 1, 2, 3, 4, 5, 6",
    standardState: "solid",
    bondingType: "covalent network",
    meltingPoint: 388,
    boilingPoint: 718,
    density: 1.96,
    groupBlock: "nonmetal",
    yearDiscovered: "Ancient",
    block: "p",
    cpkHexColor: "FFFF30",
    period: 3,
    group: 16,
  },
  {
    atomicNumber: 17,
    symbol: "Cl",
    name: "Chlorine",
    atomicMass: "35.453(2)",
    electronicConfiguration: "[Ne] 3s2 3p5",
    electronegativity: 3.16,
    atomicRadius: 99,
    ionRadius: "181 (-1)",
    vanDerWaalsRadius: 175,
    ionizationEnergy: 1251,
    electronAffinity: -349,
    oxidationStates: "-1, 1, 2, 3, 4, 5, 6, 7",
    standardState: "gas",
    bondingType: "covalent network",
    meltingPoint: 172,
    boilingPoint: 239,
    density: 0.003214,
    groupBlock: "halogen",
    yearDiscovered: 1774,
    block: "p",
    cpkHexColor: "1FF01F",
    period: 3,
    group: 17,
  },
  {
    atomicNumber: 18,
    symbol: "Ar",
    name: "Argon",
    atomicMass: "39.948(1)",
    electronicConfiguration: "[Ne] 3s2 3p6",
    electronegativity: "unknown",
    atomicRadius: 97,
    ionRadius: "unknown",
    vanDerWaalsRadius: 188,
    ionizationEnergy: 1521,
    electronAffinity: 0,
    oxidationStates: "unknown",
    standardState: "gas",
    bondingType: "atomic",
    meltingPoint: 84,
    boilingPoint: 87,
    density: 0.001784,
    groupBlock: "noble gas",
    yearDiscovered: 1894,
    block: "p",
    cpkHexColor: "80D1E3",
    period: 3,
    group: 18,
  },
  {
    atomicNumber: 19,
    symbol: "K",
    name: "Potassium",
    atomicMass: "39.0983(1)",
    electronicConfiguration: "[Ar] 4s1",
    electronegativity: 0.82,
    atomicRadius: 196,
    ionRadius: "138 (+1)",
    vanDerWaalsRadius: 275,
    ionizationEnergy: 419,
    electronAffinity: -48,
    oxidationStates: 1,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 337,
    boilingPoint: 1032,
    density: 0.856,
    groupBlock: "alkali metal",
    yearDiscovered: 1807,
    block: "s",
    cpkHexColor: "8F40D4",
    period: 4,
    group: 1,
  },
  {
    atomicNumber: 20,
    symbol: "Ca",
    name: "Calcium",
    atomicMass: "40.078(4)",
    electronicConfiguration: "[Ar] 4s2",
    electronegativity: 1,
    atomicRadius: 174,
    ionRadius: "100 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 590,
    electronAffinity: -2,
    oxidationStates: 2,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1115,
    boilingPoint: 1757,
    density: 1.55,
    groupBlock: "alkaline earth metal",
    yearDiscovered: "Ancient",
    block: "s",
    cpkHexColor: "3DFF00",
    period: 4,
    group: 2,
  },
  {
    atomicNumber: 21,
    symbol: "Sc",
    name: "Scandium",
    atomicMass: "44.955912(6)",
    electronicConfiguration: "[Ar] 3d1 4s2",
    electronegativity: 1.36,
    atomicRadius: 144,
    ionRadius: "74.5 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 633,
    electronAffinity: -18,
    oxidationStates: "1, 2, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1814,
    boilingPoint: 3103,
    density: 2.985,
    groupBlock: "transition metal",
    yearDiscovered: 1876,
    block: "d",
    cpkHexColor: "E6E6E6",
    period: 4,
    group: 3,
  },
  {
    atomicNumber: 22,
    symbol: "Ti",
    name: "Titanium",
    atomicMass: "47.867(1)",
    electronicConfiguration: "[Ar] 3d2 4s2",
    electronegativity: 1.54,
    atomicRadius: 136,
    ionRadius: "86 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 659,
    electronAffinity: -8,
    oxidationStates: "-1, 2, 3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1941,
    boilingPoint: 3560,
    density: 4.507,
    groupBlock: "transition metal",
    yearDiscovered: 1791,
    block: "d",
    cpkHexColor: "BFC2C7",
    period: 4,
    group: 4,
  },
  {
    atomicNumber: 23,
    symbol: "V",
    name: "Vanadium",
    atomicMass: "50.9415(1)",
    electronicConfiguration: "[Ar] 3d3 4s2",
    electronegativity: 1.63,
    atomicRadius: 125,
    ionRadius: "79 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 651,
    electronAffinity: -51,
    oxidationStates: "-1, 2, 3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 2183,
    boilingPoint: 3680,
    density: 6.11,
    groupBlock: "transition metal",
    yearDiscovered: 1803,
    block: "d",
    cpkHexColor: "A6A6AB",
    period: 4,
    group: 5,
  },
  {
    atomicNumber: 24,
    symbol: "Cr",
    name: "Chromium",
    atomicMass: "51.9961(6)",
    electronicConfiguration: "[Ar] 3d5 4s1",
    electronegativity: 1.66,
    atomicRadius: 127,
    ionRadius: "80 (+2*)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 653,
    electronAffinity: -64,
    oxidationStates: "-2, -1, 1, 2, 3, 4, 5, 6",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 2180,
    boilingPoint: 2944,
    density: 7.14,
    groupBlock: "transition metal",
    yearDiscovered: "Ancient",
    block: "d",
    cpkHexColor: "8A99C7",
    period: 4,
    group: 6,
  },
  {
    atomicNumber: 25,
    symbol: "Mn",
    name: "Manganese",
    atomicMass: "54.938045(5)",
    electronicConfiguration: "[Ar] 3d5 4s2",
    electronegativity: 1.55,
    atomicRadius: 139,
    ionRadius: "67 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 717,
    electronAffinity: 0,
    oxidationStates: "-3, -2, -1, 1, 2, 3, 4, 5, 6, 7",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1519,
    boilingPoint: 2334,
    density: 7.47,
    groupBlock: "transition metal",
    yearDiscovered: 1774,
    block: "d",
    cpkHexColor: "9C7AC7",
    period: 4,
    group: 7,
  },
  {
    atomicNumber: 26,
    symbol: "Fe",
    name: "Iron",
    atomicMass: "55.845(2)",
    electronicConfiguration: "[Ar] 3d6 4s2",
    electronegativity: 1.83,
    atomicRadius: 125,
    ionRadius: "78 (+2*)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 763,
    electronAffinity: -16,
    oxidationStates: "-2, -1, 1, 2, 3, 4, 5, 6",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1811,
    boilingPoint: 3134,
    density: 7.874,
    groupBlock: "transition metal",
    yearDiscovered: "Ancient",
    block: "d",
    cpkHexColor: "E06633",
    period: 4,
    group: 8,
  },
  {
    atomicNumber: 27,
    symbol: "Co",
    name: "Cobalt",
    atomicMass: "58.933195(5)",
    electronicConfiguration: "[Ar] 3d7 4s2",
    electronegativity: 1.88,
    atomicRadius: 126,
    ionRadius: "74.5 (+2*)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 760,
    electronAffinity: -64,
    oxidationStates: "-1, 1, 2, 3, 4, 5",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1768,
    boilingPoint: 3200,
    density: 8.9,
    groupBlock: "transition metal",
    yearDiscovered: "Ancient",
    block: "d",
    cpkHexColor: "F090A0",
    period: 4,
    group: 19,
  },
  {
    atomicNumber: 28,
    symbol: "Ni",
    name: "Nickel",
    atomicMass: "58.6934(4)",
    electronicConfiguration: "[Ar] 3d8 4s2",
    electronegativity: 1.91,
    atomicRadius: 121,
    ionRadius: "69 (+2)",
    vanDerWaalsRadius: 163,
    ionizationEnergy: 737,
    electronAffinity: -112,
    oxidationStates: "-1, 1, 2, 3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1728,
    boilingPoint: 3186,
    density: 8.908,
    groupBlock: "transition metal",
    yearDiscovered: 1751,
    block: "d",
    cpkHexColor: "50D050",
    period: 4,
    group: 10,
  },
  {
    atomicNumber: 29,
    symbol: "Cu",
    name: "Copper",
    atomicMass: "63.546(3)",
    electronicConfiguration: "[Ar] 3d10 4s1",
    electronegativity: 1.9,
    atomicRadius: 138,
    ionRadius: "77 (+1)",
    vanDerWaalsRadius: 140,
    ionizationEnergy: 746,
    electronAffinity: -118,
    oxidationStates: "1, 2, 3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1358,
    boilingPoint: 3200,
    density: 8.92,
    groupBlock: "transition metal",
    yearDiscovered: "Ancient",
    block: "d",
    cpkHexColor: "C88033",
    period: 4,
    group: 11,
  },
  {
    atomicNumber: 30,
    symbol: "Zn",
    name: "Zinc",
    atomicMass: "65.38(2)",
    electronicConfiguration: "[Ar] 3d10 4s2",
    electronegativity: 1.65,
    atomicRadius: 131,
    ionRadius: "74 (+2)",
    vanDerWaalsRadius: 139,
    ionizationEnergy: 906,
    electronAffinity: 0,
    oxidationStates: 2,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 693,
    boilingPoint: 1180,
    density: 7.14,
    groupBlock: "transition metal",
    yearDiscovered: 1746,
    block: "d",
    cpkHexColor: "7D80B0",
    period: 4,
    group: 12,
  },
  {
    atomicNumber: 31,
    symbol: "Ga",
    name: "Gallium",
    atomicMass: "69.723(1)",
    electronicConfiguration: "[Ar] 3d10 4s2 4p1",
    electronegativity: 1.81,
    atomicRadius: 126,
    ionRadius: "62 (+3)",
    vanDerWaalsRadius: 187,
    ionizationEnergy: 579,
    electronAffinity: -29,
    oxidationStates: "1, 2, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 303,
    boilingPoint: 2477,
    density: 5.904,
    groupBlock: "metal",
    yearDiscovered: 1875,
    block: "p",
    cpkHexColor: "C28F8F",
    period: 4,
    group: 13,
  },
  {
    atomicNumber: 32,
    symbol: "Ge",
    name: "Germanium",
    atomicMass: "72.64(1)",
    electronicConfiguration: "[Ar] 3d10 4s2 4p2",
    electronegativity: 2.01,
    atomicRadius: 122,
    ionRadius: "73 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 762,
    electronAffinity: -119,
    oxidationStates: "-4, 1, 2, 3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1211,
    boilingPoint: 3093,
    density: 5.323,
    groupBlock: "metalloid",
    yearDiscovered: 1886,
    block: "p",
    cpkHexColor: "668F8F",
    period: 4,
    group: 14,
  },
  {
    atomicNumber: 33,
    symbol: "As",
    name: "Arsenic",
    atomicMass: "74.92160(2)",
    electronicConfiguration: "[Ar] 3d10 4s2 4p3",
    electronegativity: 2.18,
    atomicRadius: 119,
    ionRadius: "58 (+3)",
    vanDerWaalsRadius: 185,
    ionizationEnergy: 947,
    electronAffinity: -78,
    oxidationStates: "-3, 2, 3, 5",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1090,
    boilingPoint: 887,
    density: 5.727,
    groupBlock: "metalloid",
    yearDiscovered: "Ancient",
    block: "p",
    cpkHexColor: "BD80E3",
    period: 4,
    group: 15,
  },
  {
    atomicNumber: 34,
    symbol: "Se",
    name: "Selenium",
    atomicMass: "78.96(3)",
    electronicConfiguration: "[Ar] 3d10 4s2 4p4",
    electronegativity: 2.55,
    atomicRadius: 116,
    ionRadius: "198 (-2)",
    vanDerWaalsRadius: 190,
    ionizationEnergy: 941,
    electronAffinity: -195,
    oxidationStates: "-2, 2, 4, 6",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 494,
    boilingPoint: 958,
    density: 4.819,
    groupBlock: "nonmetal",
    yearDiscovered: 1817,
    block: "p",
    cpkHexColor: "FFA100",
    period: 4,
    group: 16,
  },
  {
    atomicNumber: 35,
    symbol: "Br",
    name: "Bromine",
    atomicMass: "79.904(1)",
    electronicConfiguration: "[Ar] 3d10 4s2 4p5",
    electronegativity: 2.96,
    atomicRadius: 114,
    ionRadius: "196 (-1)",
    vanDerWaalsRadius: 185,
    ionizationEnergy: 1140,
    electronAffinity: -325,
    oxidationStates: "-1, 1, 3, 4, 5, 7",
    standardState: "liquid",
    bondingType: "covalent network",
    meltingPoint: 266,
    boilingPoint: 332,
    density: 3.12,
    groupBlock: "halogen",
    yearDiscovered: 1826,
    block: "p",
    cpkHexColor: "A62929",
    period: 4,
    group: 17,
  },
  {
    atomicNumber: 36,
    symbol: "Kr",
    name: "Krypton",
    atomicMass: "83.798(2)",
    electronicConfiguration: "[Ar] 3d10 4s2 4p6",
    electronegativity: "unknown",
    atomicRadius: 110,
    ionRadius: "unknown",
    vanDerWaalsRadius: 202,
    ionizationEnergy: 1351,
    electronAffinity: 0,
    oxidationStates: 2,
    standardState: "gas",
    bondingType: "atomic",
    meltingPoint: 116,
    boilingPoint: 120,
    density: 0.00375,
    groupBlock: "noble gas",
    yearDiscovered: 1898,
    block: "p",
    cpkHexColor: "5CB8D1",
    period: 4,
    group: 18,
  },
  {
    atomicNumber: 37,
    symbol: "Rb",
    name: "Rubidium",
    atomicMass: "85.4678(3)",
    electronicConfiguration: "[Kr] 5s1",
    electronegativity: 0.82,
    atomicRadius: 211,
    ionRadius: "152 (+1)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 403,
    electronAffinity: -47,
    oxidationStates: 1,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 312,
    boilingPoint: 961,
    density: 1.532,
    groupBlock: "alkali metal",
    yearDiscovered: 1861,
    block: "s",
    cpkHexColor: "702EB0",
    period: 5,
    group: 1,
  },
  {
    atomicNumber: 38,
    symbol: "Sr",
    name: "Strontium",
    atomicMass: "87.62(1)",
    electronicConfiguration: "[Kr] 5s2",
    electronegativity: 0.95,
    atomicRadius: 192,
    ionRadius: "118 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 550,
    electronAffinity: -5,
    oxidationStates: 2,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1050,
    boilingPoint: 1655,
    density: 2.63,
    groupBlock: "alkaline earth metal",
    yearDiscovered: 1790,
    block: "s",
    cpkHexColor: "00FF00",
    period: 5,
    group: 2,
  },
  {
    atomicNumber: 39,
    symbol: "Y",
    name: "Yttrium",
    atomicMass: "88.90585(2)",
    electronicConfiguration: "[Kr] 4d1 5s2",
    electronegativity: 1.22,
    atomicRadius: 162,
    ionRadius: "90 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 600,
    electronAffinity: -30,
    oxidationStates: "1, 2, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1799,
    boilingPoint: 3618,
    density: 4.472,
    groupBlock: "transition metal",
    yearDiscovered: 1794,
    block: "d",
    cpkHexColor: "94FFFF",
    period: 5,
    group: 3,
  },
  {
    atomicNumber: 40,
    symbol: "Zr",
    name: "Zirconium",
    atomicMass: "91.224(2)",
    electronicConfiguration: "[Kr] 4d2 5s2",
    electronegativity: 1.33,
    atomicRadius: 148,
    ionRadius: "72 (+4)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 640,
    electronAffinity: -41,
    oxidationStates: "1, 2, 3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 2128,
    boilingPoint: 4682,
    density: 6.511,
    groupBlock: "transition metal",
    yearDiscovered: 1789,
    block: "d",
    cpkHexColor: "94E0E0",
    period: 5,
    group: 4,
  },
  {
    atomicNumber: 41,
    symbol: "Nb",
    name: "Niobium",
    atomicMass: "92.90638(2)",
    electronicConfiguration: "[Kr] 4d4 5s1",
    electronegativity: 1.6,
    atomicRadius: 137,
    ionRadius: "72 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 652,
    electronAffinity: -86,
    oxidationStates: "-1, 2, 3, 4, 5",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 2750,
    boilingPoint: 5017,
    density: 8.57,
    groupBlock: "transition metal",
    yearDiscovered: 1801,
    block: "d",
    cpkHexColor: "73C2C9",
    period: 5,
    group: 5,
  },
  {
    atomicNumber: 42,
    symbol: "Mo",
    name: "Molybdenum",
    atomicMass: "95.96(2)",
    electronicConfiguration: "[Kr] 4d5 5s1",
    electronegativity: 2.16,
    atomicRadius: 145,
    ionRadius: "69 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 684,
    electronAffinity: -72,
    oxidationStates: "-2, -1, 1, 2, 3, 4, 5, 6",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 2896,
    boilingPoint: 4912,
    density: 10.28,
    groupBlock: "transition metal",
    yearDiscovered: 1778,
    block: "d",
    cpkHexColor: "54B5B5",
    period: 5,
    group: 6,
  },
  {
    atomicNumber: 43,
    symbol: "Tc",
    name: "Technetium",
    atomicMass: [98],
    electronicConfiguration: "[Kr] 4d5 5s2",
    electronegativity: 1.9,
    atomicRadius: 156,
    ionRadius: "64.5 (+4)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 702,
    electronAffinity: -53,
    oxidationStates: "-3, -1, 1, 2, 3, 4, 5, 6, 7",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 2430,
    boilingPoint: 4538,
    density: 11.5,
    groupBlock: "transition metal",
    yearDiscovered: 1937,
    block: "d",
    cpkHexColor: "3B9E9E",
    period: 5,
    group: 7,
  },
  {
    atomicNumber: 44,
    symbol: "Ru",
    name: "Ruthenium",
    atomicMass: "101.07(2)",
    electronicConfiguration: "[Kr] 4d7 5s1",
    electronegativity: 2.2,
    atomicRadius: 126,
    ionRadius: "68 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 710,
    electronAffinity: -101,
    oxidationStates: "-2, 1, 2, 3, 4, 5, 6, 7, 8",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 2607,
    boilingPoint: 4423,
    density: 12.37,
    groupBlock: "transition metal",
    yearDiscovered: 1827,
    block: "d",
    cpkHexColor: "248F8F",
    period: 5,
    group: 8,
  },
  {
    atomicNumber: 45,
    symbol: "Rh",
    name: "Rhodium",
    atomicMass: "102.90550(2)",
    electronicConfiguration: "[Kr] 4d8 5s1",
    electronegativity: 2.28,
    atomicRadius: 135,
    ionRadius: "66.5 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 720,
    electronAffinity: -110,
    oxidationStates: "-1, 1, 2, 3, 4, 5, 6",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 2237,
    boilingPoint: 3968,
    density: 12.45,
    groupBlock: "transition metal",
    yearDiscovered: 1803,
    block: "d",
    cpkHexColor: "0A7D8C",
    period: 5,
    group: 9,
  },
  {
    atomicNumber: 46,
    symbol: "Pd",
    name: "Palladium",
    atomicMass: "106.42(1)",
    electronicConfiguration: "[Kr] 4d10",
    electronegativity: 2.2,
    atomicRadius: 131,
    ionRadius: "59 (+1)",
    vanDerWaalsRadius: 163,
    ionizationEnergy: 804,
    electronAffinity: -54,
    oxidationStates: "2, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1828,
    boilingPoint: 3236,
    density: 12.023,
    groupBlock: "transition metal",
    yearDiscovered: 1803,
    block: "d",
    cpkHexColor: "6985",
    period: 5,
    group: 10,
  },
  {
    atomicNumber: 47,
    symbol: "Ag",
    name: "Silver",
    atomicMass: "107.8682(2)",
    electronicConfiguration: "[Kr] 4d10 5s1",
    electronegativity: 1.93,
    atomicRadius: 153,
    ionRadius: "115 (+1)",
    vanDerWaalsRadius: 172,
    ionizationEnergy: 731,
    electronAffinity: -126,
    oxidationStates: "1, 2, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1235,
    boilingPoint: 2435,
    density: 10.49,
    groupBlock: "transition metal",
    yearDiscovered: "Ancient",
    block: "d",
    cpkHexColor: "C0C0C0",
    period: 5,
    group: 11,
  },
  {
    atomicNumber: 48,
    symbol: "Cd",
    name: "Cadmium",
    atomicMass: "112.411(8)",
    electronicConfiguration: "[Kr] 4d10 5s2",
    electronegativity: 1.69,
    atomicRadius: 148,
    ionRadius: "95 (+2)",
    vanDerWaalsRadius: 158,
    ionizationEnergy: 868,
    electronAffinity: 0,
    oxidationStates: 2,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 594,
    boilingPoint: 1040,
    density: 8.65,
    groupBlock: "transition metal",
    yearDiscovered: 1817,
    block: "d",
    cpkHexColor: "FFD98F",
    period: 5,
    group: 12,
  },
  {
    atomicNumber: 49,
    symbol: "In",
    name: "Indium",
    atomicMass: "114.818(3)",
    electronicConfiguration: "[Kr] 4d10 5s2 5p1",
    electronegativity: 1.78,
    atomicRadius: 144,
    ionRadius: "80 (+3)",
    vanDerWaalsRadius: 193,
    ionizationEnergy: 558,
    electronAffinity: -29,
    oxidationStates: "1, 2, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 430,
    boilingPoint: 2345,
    density: 7.31,
    groupBlock: "metal",
    yearDiscovered: 1863,
    block: "p",
    cpkHexColor: "A67573",
    period: 5,
    group: 13,
  },
  {
    atomicNumber: 50,
    symbol: "Sn",
    name: "Tin",
    atomicMass: "118.710(7)",
    electronicConfiguration: "[Kr] 4d10 5s2 5p2",
    electronegativity: 1.96,
    atomicRadius: 141,
    ionRadius: "112 (+2)",
    vanDerWaalsRadius: 217,
    ionizationEnergy: 709,
    electronAffinity: -107,
    oxidationStates: "-4, 2, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 505,
    boilingPoint: 2875,
    density: 7.31,
    groupBlock: "metal",
    yearDiscovered: "Ancient",
    block: "p",
    cpkHexColor: "668080",
    period: 5,
    group: 14,
  },
  {
    atomicNumber: 51,
    symbol: "Sb",
    name: "Antimony",
    atomicMass: "121.760(1)",
    electronicConfiguration: "[Kr] 4d10 5s2 5p3",
    electronegativity: 2.05,
    atomicRadius: 138,
    ionRadius: "76 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 834,
    electronAffinity: -103,
    oxidationStates: "-3, 3, 5",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 904,
    boilingPoint: 1860,
    density: 6.697,
    groupBlock: "metalloid",
    yearDiscovered: "Ancient",
    block: "p",
    cpkHexColor: "9E63B5",
    period: 5,
    group: 15,
  },
  {
    atomicNumber: 52,
    symbol: "Te",
    name: "Tellurium",
    atomicMass: "127.60(3)",
    electronicConfiguration: "[Kr] 4d10 5s2 5p4",
    electronegativity: 2.1,
    atomicRadius: 135,
    ionRadius: "221 (-2)",
    vanDerWaalsRadius: 206,
    ionizationEnergy: 869,
    electronAffinity: -190,
    oxidationStates: "-2, 2, 4, 5, 6",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 723,
    boilingPoint: 1261,
    density: 6.24,
    groupBlock: "metalloid",
    yearDiscovered: 1782,
    block: "p",
    cpkHexColor: "D47A00",
    period: 5,
    group: 16,
  },
  {
    atomicNumber: 53,
    symbol: "I",
    name: "Iodine",
    atomicMass: "126.90447(3)",
    electronicConfiguration: "[Kr] 4d10 5s2 5p5",
    electronegativity: 2.66,
    atomicRadius: 133,
    ionRadius: "220 (-1)",
    vanDerWaalsRadius: 198,
    ionizationEnergy: 1008,
    electronAffinity: -295,
    oxidationStates: "-1, 1, 3, 5, 7",
    standardState: "solid",
    bondingType: "covalent network",
    meltingPoint: 387,
    boilingPoint: 457,
    density: 4.94,
    groupBlock: "halogen",
    yearDiscovered: 1811,
    block: "p",
    cpkHexColor: "940094",
    period: 5,
    group: 17,
  },
  {
    atomicNumber: 54,
    symbol: "Xe",
    name: "Xenon",
    atomicMass: "131.293(6)",
    electronicConfiguration: "[Kr] 4d10 5s2 5p6",
    electronegativity: "unknown",
    atomicRadius: 130,
    ionRadius: "48 (+8)",
    vanDerWaalsRadius: 216,
    ionizationEnergy: 1170,
    electronAffinity: 0,
    oxidationStates: "2, 4, 6, 8",
    standardState: "gas",
    bondingType: "atomic",
    meltingPoint: 161,
    boilingPoint: 165,
    density: 0.0059,
    groupBlock: "noble gas",
    yearDiscovered: 1898,
    block: "p",
    cpkHexColor: "429EB0",
    period: 5,
    group: 18,
  },
  {
    atomicNumber: 55,
    symbol: "Cs",
    name: "Cesium",
    atomicMass: "132.9054519(2)",
    electronicConfiguration: "[Xe] 6s1",
    electronegativity: 0.79,
    atomicRadius: 225,
    ionRadius: "167 (+1)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 376,
    electronAffinity: -46,
    oxidationStates: 1,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 302,
    boilingPoint: 944,
    density: 1.879,
    groupBlock: "alkali metal",
    yearDiscovered: 1860,
    block: "s",
    cpkHexColor: "57178F",
    period: 6,
    group: 1,
  },
  {
    atomicNumber: 56,
    symbol: "Ba",
    name: "Barium",
    atomicMass: "137.327(7)",
    electronicConfiguration: "[Xe] 6s2",
    electronegativity: 0.89,
    atomicRadius: 198,
    ionRadius: "135 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 503,
    electronAffinity: -14,
    oxidationStates: 2,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1000,
    boilingPoint: 2143,
    density: 3.51,
    groupBlock: "alkaline earth metal",
    yearDiscovered: 1808,
    block: "s",
    cpkHexColor: "00C900",
    period: 6,
    group: 2,
  },
  {
    atomicNumber: 57,
    symbol: "La",
    name: "Lanthanum",
    atomicMass: "138.90547(7)",
    electronicConfiguration: "[Xe] 5d1 6s2",
    electronegativity: 1.1,
    atomicRadius: 169,
    ionRadius: "103.2 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 538,
    electronAffinity: -48,
    oxidationStates: "2, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1193,
    boilingPoint: 3737,
    density: 6.146,
    groupBlock: "lanthanoid",
    yearDiscovered: 1839,
    block: "f",
    cpkHexColor: "70D4FF",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 58,
    symbol: "Ce",
    name: "Cerium",
    atomicMass: "140.116(1)",
    electronicConfiguration: "[Xe] 4f1 5d1 6s2",
    electronegativity: 1.12,
    atomicRadius: "unknown",
    ionRadius: "102 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 534,
    electronAffinity: -50,
    oxidationStates: "2, 3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1071,
    boilingPoint: 3633,
    density: 6.689,
    groupBlock: "lanthanoid",
    yearDiscovered: 1803,
    block: "f",
    cpkHexColor: "FFFFC7",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 59,
    symbol: "Pr",
    name: "Praseodymium",
    atomicMass: "140.90765(2)",
    electronicConfiguration: "[Xe] 4f3 6s2",
    electronegativity: 1.13,
    atomicRadius: "unknown",
    ionRadius: "99 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 527,
    electronAffinity: -50,
    oxidationStates: "2, 3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1204,
    boilingPoint: 3563,
    density: 6.64,
    groupBlock: "lanthanoid",
    yearDiscovered: 1885,
    block: "f",
    cpkHexColor: "D9FFC7",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 60,
    symbol: "Nd",
    name: "Neodymium",
    atomicMass: "144.242(3)",
    electronicConfiguration: "[Xe] 4f4 6s2",
    electronegativity: 1.14,
    atomicRadius: "unknown",
    ionRadius: "129 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 533,
    electronAffinity: -50,
    oxidationStates: "2, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1294,
    boilingPoint: 3373,
    density: 7.01,
    groupBlock: "lanthanoid",
    yearDiscovered: 1885,
    block: "f",
    cpkHexColor: "C7FFC7",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 61,
    symbol: "Pm",
    name: "Promethium",
    atomicMass: [145],
    electronicConfiguration: "[Xe] 4f5 6s2",
    electronegativity: 1.13,
    atomicRadius: "unknown",
    ionRadius: "97 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 540,
    electronAffinity: -50,
    oxidationStates: 3,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1373,
    boilingPoint: 3273,
    density: 7.264,
    groupBlock: "lanthanoid",
    yearDiscovered: 1947,
    block: "f",
    cpkHexColor: "A3FFC7",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 62,
    symbol: "Sm",
    name: "Samarium",
    atomicMass: "150.36(2)",
    electronicConfiguration: "[Xe] 4f6 6s2",
    electronegativity: 1.17,
    atomicRadius: "unknown",
    ionRadius: "122 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 545,
    electronAffinity: -50,
    oxidationStates: "2, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1345,
    boilingPoint: 2076,
    density: 7.353,
    groupBlock: "lanthanoid",
    yearDiscovered: 1853,
    block: "f",
    cpkHexColor: "8FFFC7",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 63,
    symbol: "Eu",
    name: "Europium",
    atomicMass: "151.964(1)",
    electronicConfiguration: "[Xe] 4f7 6s2",
    electronegativity: 1.2,
    atomicRadius: "unknown",
    ionRadius: "117 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 547,
    electronAffinity: -50,
    oxidationStates: "2, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1095,
    boilingPoint: 1800,
    density: 5.244,
    groupBlock: "lanthanoid",
    yearDiscovered: 1901,
    block: "f",
    cpkHexColor: "61FFC7",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 64,
    symbol: "Gd",
    name: "Gadolinium",
    atomicMass: "157.25(3)",
    electronicConfiguration: "[Xe] 4f7 5d1 6s2",
    electronegativity: 1.2,
    atomicRadius: "unknown",
    ionRadius: "93.8 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 593,
    electronAffinity: -50,
    oxidationStates: "1, 2, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1586,
    boilingPoint: 3523,
    density: 7.901,
    groupBlock: "lanthanoid",
    yearDiscovered: 1880,
    block: "f",
    cpkHexColor: "45FFC7",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 65,
    symbol: "Tb",
    name: "Terbium",
    atomicMass: "158.92535(2)",
    electronicConfiguration: "[Xe] 4f9 6s2",
    electronegativity: 1.2,
    atomicRadius: "unknown",
    ionRadius: "92.3 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 566,
    electronAffinity: -50,
    oxidationStates: "1, 3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1629,
    boilingPoint: 3503,
    density: 8.219,
    groupBlock: "lanthanoid",
    yearDiscovered: 1843,
    block: "f",
    cpkHexColor: "30FFC7",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 66,
    symbol: "Dy",
    name: "Dysprosium",
    atomicMass: "162.500(1)",
    electronicConfiguration: "[Xe] 4f10 6s2",
    electronegativity: 1.22,
    atomicRadius: "unknown",
    ionRadius: "107 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 573,
    electronAffinity: -50,
    oxidationStates: "2, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1685,
    boilingPoint: 2840,
    density: 8.551,
    groupBlock: "lanthanoid",
    yearDiscovered: 1886,
    block: "f",
    cpkHexColor: "1FFFC7",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 67,
    symbol: "Ho",
    name: "Holmium",
    atomicMass: "164.93032(2)",
    electronicConfiguration: "[Xe] 4f11 6s2",
    electronegativity: 1.23,
    atomicRadius: "unknown",
    ionRadius: "90.1 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 581,
    electronAffinity: -50,
    oxidationStates: 3,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1747,
    boilingPoint: 2973,
    density: 8.795,
    groupBlock: "lanthanoid",
    yearDiscovered: 1878,
    block: "f",
    cpkHexColor: "00FF9C",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 68,
    symbol: "Er",
    name: "Erbium",
    atomicMass: "167.259(3)",
    electronicConfiguration: "[Xe] 4f12 6s2",
    electronegativity: 1.24,
    atomicRadius: "unknown",
    ionRadius: "89 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 589,
    electronAffinity: -50,
    oxidationStates: 3,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1770,
    boilingPoint: 3141,
    density: 9.066,
    groupBlock: "lanthanoid",
    yearDiscovered: 1842,
    block: "f",
    cpkHexColor: "0",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 69,
    symbol: "Tm",
    name: "Thulium",
    atomicMass: "168.93421(2)",
    electronicConfiguration: "[Xe] 4f13 6s2",
    electronegativity: 1.25,
    atomicRadius: "unknown",
    ionRadius: "103 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 597,
    electronAffinity: -50,
    oxidationStates: "2, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1818,
    boilingPoint: 2223,
    density: 9.321,
    groupBlock: "lanthanoid",
    yearDiscovered: 1879,
    block: "f",
    cpkHexColor: "00D452",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 70,
    symbol: "Yb",
    name: "Ytterbium",
    atomicMass: "173.054(5)",
    electronicConfiguration: "[Xe] 4f14 6s2",
    electronegativity: 1.1,
    atomicRadius: "unknown",
    ionRadius: "102 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 603,
    electronAffinity: -50,
    oxidationStates: "2, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1092,
    boilingPoint: 1469,
    density: 6.57,
    groupBlock: "lanthanoid",
    yearDiscovered: 1878,
    block: "f",
    cpkHexColor: "00BF38",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 71,
    symbol: "Lu",
    name: "Lutetium",
    atomicMass: "174.9668(1)",
    electronicConfiguration: "[Xe] 4f14 5d1 6s2",
    electronegativity: 1.27,
    atomicRadius: 160,
    ionRadius: "86.1 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 524,
    electronAffinity: -50,
    oxidationStates: 3,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1936,
    boilingPoint: 3675,
    density: 9.841,
    groupBlock: "lanthanoid",
    yearDiscovered: 1907,
    block: "d",
    cpkHexColor: "00AB24",
    period: 6,
    group: 3,
  },
  {
    atomicNumber: 72,
    symbol: "Hf",
    name: "Hafnium",
    atomicMass: "178.49(2)",
    electronicConfiguration: "[Xe] 4f14 5d2 6s2",
    electronegativity: 1.3,
    atomicRadius: 150,
    ionRadius: "71 (+4)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 659,
    electronAffinity: 0,
    oxidationStates: "2, 3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 2506,
    boilingPoint: 4876,
    density: 13.31,
    groupBlock: "transition metal",
    yearDiscovered: 1923,
    block: "d",
    cpkHexColor: "4DC2FF",
    period: 6,
    group: 4,
  },
  {
    atomicNumber: 73,
    symbol: "Ta",
    name: "Tantalum",
    atomicMass: "180.94788(2)",
    electronicConfiguration: "[Xe] 4f14 5d3 6s2",
    electronegativity: 1.5,
    atomicRadius: 138,
    ionRadius: "72 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 761,
    electronAffinity: -31,
    oxidationStates: "-1, 2, 3, 4, 5",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 3290,
    boilingPoint: 5731,
    density: 16.65,
    groupBlock: "transition metal",
    yearDiscovered: 1802,
    block: "d",
    cpkHexColor: "4DA6FF",
    period: 6,
    group: 5,
  },
  {
    atomicNumber: 74,
    symbol: "W",
    name: "Tungsten",
    atomicMass: "183.84(1)",
    electronicConfiguration: "[Xe] 4f14 5d4 6s2",
    electronegativity: 2.36,
    atomicRadius: 146,
    ionRadius: "66 (+4)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 770,
    electronAffinity: -79,
    oxidationStates: "-2, -1, 1, 2, 3, 4, 5, 6",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 3695,
    boilingPoint: 5828,
    density: 19.25,
    groupBlock: "transition metal",
    yearDiscovered: 1783,
    block: "d",
    cpkHexColor: "2194D6",
    period: 6,
    group: 6,
  },
  {
    atomicNumber: 75,
    symbol: "Re",
    name: "Rhenium",
    atomicMass: "186.207(1)",
    electronicConfiguration: "[Xe] 4f14 5d5 6s2",
    electronegativity: 1.9,
    atomicRadius: 159,
    ionRadius: "63 (+4)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 760,
    electronAffinity: -15,
    oxidationStates: "-3, -1, 1, 2, 3, 4, 5, 6, 7",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 3459,
    boilingPoint: 5869,
    density: 21.02,
    groupBlock: "transition metal",
    yearDiscovered: 1925,
    block: "d",
    cpkHexColor: "267DAB",
    period: 6,
    group: 7,
  },
  {
    atomicNumber: 76,
    symbol: "Os",
    name: "Osmium",
    atomicMass: "190.23(3)",
    electronicConfiguration: "[Xe] 4f14 5d6 6s2",
    electronegativity: 2.2,
    atomicRadius: 128,
    ionRadius: "63 (+4)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 840,
    electronAffinity: -106,
    oxidationStates: "-2, -1, 1, 2, 3, 4, 5, 6, 7, 8",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 3306,
    boilingPoint: 5285,
    density: 22.61,
    groupBlock: "transition metal",
    yearDiscovered: 1803,
    block: "d",
    cpkHexColor: "266696",
    period: 6,
    group: 8,
  },
  {
    atomicNumber: 77,
    symbol: "Ir",
    name: "Iridium",
    atomicMass: "192.217(3)",
    electronicConfiguration: "[Xe] 4f14 5d7 6s2",
    electronegativity: 2.2,
    atomicRadius: 137,
    ionRadius: "68 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 880,
    electronAffinity: -151,
    oxidationStates: "-3, -1, 1, 2, 3, 4, 5, 6",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 2739,
    boilingPoint: 4701,
    density: 22.65,
    groupBlock: "transition metal",
    yearDiscovered: 1803,
    block: "d",
    cpkHexColor: "175487",
    period: 6,
    group: 9,
  },
  {
    atomicNumber: 78,
    symbol: "Pt",
    name: "Platinum",
    atomicMass: "195.084(9)",
    electronicConfiguration: "[Xe] 4f14 5d9 6s1",
    electronegativity: 2.28,
    atomicRadius: 128,
    ionRadius: "86 (+2)",
    vanDerWaalsRadius: 175,
    ionizationEnergy: 870,
    electronAffinity: -205,
    oxidationStates: "2, 4, 5, 6",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 2041,
    boilingPoint: 4098,
    density: 21.09,
    groupBlock: "transition metal",
    yearDiscovered: "Ancient",
    block: "d",
    cpkHexColor: "D0D0E0",
    period: 6,
    group: 10,
  },
  {
    atomicNumber: 79,
    symbol: "Au",
    name: "Gold",
    atomicMass: "196.966569(4)",
    electronicConfiguration: "[Xe] 4f14 5d10 6s1",
    electronegativity: 2.54,
    atomicRadius: 144,
    ionRadius: "137 (+1)",
    vanDerWaalsRadius: 166,
    ionizationEnergy: 890,
    electronAffinity: -223,
    oxidationStates: "-1, 1, 2, 3, 5",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1337,
    boilingPoint: 3129,
    density: 19.3,
    groupBlock: "transition metal",
    yearDiscovered: "Ancient",
    block: "d",
    cpkHexColor: "FFD123",
    period: 6,
    group: 11,
  },
  {
    atomicNumber: 80,
    symbol: "Hg",
    name: "Mercury",
    atomicMass: "200.59(2)",
    electronicConfiguration: "[Xe] 4f14 5d10 6s2",
    electronegativity: 2,
    atomicRadius: 149,
    ionRadius: "119 (+1)",
    vanDerWaalsRadius: 155,
    ionizationEnergy: 1007,
    electronAffinity: 0,
    oxidationStates: "1, 2, 4",
    standardState: "liquid",
    bondingType: "metallic",
    meltingPoint: 234,
    boilingPoint: 630,
    density: 13.534,
    groupBlock: "transition metal",
    yearDiscovered: "Ancient",
    block: "d",
    cpkHexColor: "B8B8D0",
    period: 6,
    group: 12,
  },
  {
    atomicNumber: 81,
    symbol: "Tl",
    name: "Thallium",
    atomicMass: "204.3833(2)",
    electronicConfiguration: "[Xe] 4f14 5d10 6s2 6p1",
    electronegativity: 2.04,
    atomicRadius: 148,
    ionRadius: "150 (+1)",
    vanDerWaalsRadius: 196,
    ionizationEnergy: 589,
    electronAffinity: -19,
    oxidationStates: "1, 3",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 577,
    boilingPoint: 1746,
    density: 11.85,
    groupBlock: "metal",
    yearDiscovered: 1861,
    block: "p",
    cpkHexColor: "A6544D",
    period: 6,
    group: 13,
  },
  {
    atomicNumber: 82,
    symbol: "Pb",
    name: "Lead",
    atomicMass: "207.2(1)",
    electronicConfiguration: "[Xe] 4f14 5d10 6s2 6p2",
    electronegativity: 2.33,
    atomicRadius: 147,
    ionRadius: "119 (+2)",
    vanDerWaalsRadius: 202,
    ionizationEnergy: 716,
    electronAffinity: -35,
    oxidationStates: "-4, 2, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 601,
    boilingPoint: 2022,
    density: 11.34,
    groupBlock: "metal",
    yearDiscovered: "Ancient",
    block: "p",
    cpkHexColor: "575961",
    period: 6,
    group: 14,
  },
  {
    atomicNumber: 83,
    symbol: "Bi",
    name: "Bismuth",
    atomicMass: "208.98040(1)",
    electronicConfiguration: "[Xe] 4f14 5d10 6s2 6p3",
    electronegativity: 2.02,
    atomicRadius: 146,
    ionRadius: "103 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 703,
    electronAffinity: -91,
    oxidationStates: "-3, 3, 5",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 544,
    boilingPoint: 1837,
    density: 9.78,
    groupBlock: "metal",
    yearDiscovered: "Ancient",
    block: "p",
    cpkHexColor: "9E4FB5",
    period: 6,
    group: 15,
  },
  {
    atomicNumber: 84,
    symbol: "Po",
    name: "Polonium",
    atomicMass: [209],
    electronicConfiguration: "[Xe] 4f14 5d10 6s2 6p4",
    electronegativity: 2,
    atomicRadius: "unknown",
    ionRadius: "94 (+4)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 812,
    electronAffinity: -183,
    oxidationStates: "-2, 2, 4, 6",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 527,
    boilingPoint: 1235,
    density: 9.196,
    groupBlock: "metalloid",
    yearDiscovered: 1898,
    block: "p",
    cpkHexColor: "AB5C00",
    period: 6,
    group: 16,
  },
  {
    atomicNumber: 85,
    symbol: "At",
    name: "Astatine",
    atomicMass: [210],
    electronicConfiguration: "[Xe] 4f14 5d10 6s2 6p5",
    electronegativity: 2.2,
    atomicRadius: "unknown",
    ionRadius: "62 (+7)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 920,
    electronAffinity: -270,
    oxidationStates: "-1, 1, 3, 5",
    standardState: "solid",
    bondingType: "covalent network",
    meltingPoint: 575,
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "halogen",
    yearDiscovered: 1940,
    block: "p",
    cpkHexColor: "754F45",
    period: 6,
    group: 17,
  },
  {
    atomicNumber: 86,
    symbol: "Rn",
    name: "Radon",
    atomicMass: [222],
    electronicConfiguration: "[Xe] 4f14 5d10 6s2 6p6",
    electronegativity: "unknown",
    atomicRadius: 145,
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 1037,
    electronAffinity: "unknown",
    oxidationStates: 2,
    standardState: "gas",
    bondingType: "atomic",
    meltingPoint: 202,
    boilingPoint: 211,
    density: 0.00973,
    groupBlock: "noble gas",
    yearDiscovered: 1900,
    block: "p",
    cpkHexColor: "428296",
    period: 6,
    group: 18,
  },
  {
    atomicNumber: 87,
    symbol: "Fr",
    name: "Francium",
    atomicMass: [223],
    electronicConfiguration: "[Rn] 7s1",
    electronegativity: 0.7,
    atomicRadius: "unknown",
    ionRadius: "180 (+1)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 380,
    electronAffinity: "unknown",
    oxidationStates: 1,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "alkali metal",
    yearDiscovered: 1939,
    block: "s",
    cpkHexColor: "420066",
    period: 7,
    group: 1,
  },
  {
    atomicNumber: 88,
    symbol: "Ra",
    name: "Radium",
    atomicMass: [226],
    electronicConfiguration: "[Rn] 7s2",
    electronegativity: 0.9,
    atomicRadius: "unknown",
    ionRadius: "148 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 509,
    electronAffinity: "unknown",
    oxidationStates: 2,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 973,
    boilingPoint: 2010,
    density: 5,
    groupBlock: "alkaline earth metal",
    yearDiscovered: 1898,
    block: "s",
    cpkHexColor: "007D00",
    period: 7,
    group: 2,
  },
  {
    atomicNumber: 89,
    symbol: "Ac",
    name: "Actinium",
    atomicMass: [227],
    electronicConfiguration: "[Rn] 6d1 7s2",
    electronegativity: 1.1,
    atomicRadius: "unknown",
    ionRadius: "112 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 499,
    electronAffinity: "unknown",
    oxidationStates: 3,
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1323,
    boilingPoint: 3473,
    density: 10.07,
    groupBlock: "actinoid",
    yearDiscovered: 1899,
    block: "f",
    cpkHexColor: "70ABFA",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 90,
    symbol: "Th",
    name: "Thorium",
    atomicMass: "232.03806(2)",
    electronicConfiguration: "[Rn] 6d2 7s2",
    electronegativity: 1.3,
    atomicRadius: "unknown",
    ionRadius: "94 (+4)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 587,
    electronAffinity: "unknown",
    oxidationStates: "2, 3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 2023,
    boilingPoint: 5093,
    density: 11.724,
    groupBlock: "actinoid",
    yearDiscovered: 1828,
    block: "f",
    cpkHexColor: "00BAFF",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 91,
    symbol: "Pa",
    name: "Protactinium",
    atomicMass: "231.03588(2)",
    electronicConfiguration: "[Rn] 5f2 6d1 7s2",
    electronegativity: 1.5,
    atomicRadius: "unknown",
    ionRadius: "104 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 568,
    electronAffinity: "unknown",
    oxidationStates: "3, 4, 5",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1845,
    boilingPoint: 4273,
    density: 15.37,
    groupBlock: "actinoid",
    yearDiscovered: 1913,
    block: "f",
    cpkHexColor: "00A1FF",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 92,
    symbol: "U",
    name: "Uranium",
    atomicMass: "238.02891(3)",
    electronicConfiguration: "[Rn] 5f3 6d1 7s2",
    electronegativity: 1.38,
    atomicRadius: "unknown",
    ionRadius: "102.5 (+3)",
    vanDerWaalsRadius: 186,
    ionizationEnergy: 598,
    electronAffinity: "unknown",
    oxidationStates: "3, 4, 5, 6",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1408,
    boilingPoint: 4200,
    density: 19.05,
    groupBlock: "actinoid",
    yearDiscovered: 1789,
    block: "f",
    cpkHexColor: "008FFF",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 93,
    symbol: "Np",
    name: "Neptunium",
    atomicMass: [237],
    electronicConfiguration: "[Rn] 5f4 6d1 7s2",
    electronegativity: 1.36,
    atomicRadius: "unknown",
    ionRadius: "110 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 605,
    electronAffinity: "unknown",
    oxidationStates: "3, 4, 5, 6, 7",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 917,
    boilingPoint: 4273,
    density: 20.45,
    groupBlock: "actinoid",
    yearDiscovered: 1940,
    block: "f",
    cpkHexColor: "0080FF",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 94,
    symbol: "Pu",
    name: "Plutonium",
    atomicMass: [244],
    electronicConfiguration: "[Rn] 5f6 7s2",
    electronegativity: 1.28,
    atomicRadius: "unknown",
    ionRadius: "100 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 585,
    electronAffinity: "unknown",
    oxidationStates: "3, 4, 5, 6, 7",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 913,
    boilingPoint: 3503,
    density: 19.816,
    groupBlock: "actinoid",
    yearDiscovered: 1940,
    block: "f",
    cpkHexColor: "006BFF",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 95,
    symbol: "Am",
    name: "Americium",
    atomicMass: [243],
    electronicConfiguration: "[Rn] 5f7 7s2",
    electronegativity: 1.3,
    atomicRadius: "unknown",
    ionRadius: "126 (+2)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 578,
    electronAffinity: "unknown",
    oxidationStates: "2, 3, 4, 5, 6",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1449,
    boilingPoint: 2284,
    density: "unknown",
    groupBlock: "actinoid",
    yearDiscovered: 1944,
    block: "f",
    cpkHexColor: "545CF2",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 96,
    symbol: "Cm",
    name: "Curium",
    atomicMass: [247],
    electronicConfiguration: "[Rn] 5f7 6d1 7s2",
    electronegativity: 1.3,
    atomicRadius: "unknown",
    ionRadius: "97 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 581,
    electronAffinity: "unknown",
    oxidationStates: "3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1618,
    boilingPoint: 3383,
    density: 13.51,
    groupBlock: "actinoid",
    yearDiscovered: 1944,
    block: "f",
    cpkHexColor: "785CE3",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 97,
    symbol: "Bk",
    name: "Berkelium",
    atomicMass: [247],
    electronicConfiguration: "[Rn] 5f9 7s2",
    electronegativity: 1.3,
    atomicRadius: "unknown",
    ionRadius: "96 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 601,
    electronAffinity: "unknown",
    oxidationStates: "3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1323,
    boilingPoint: "unknown",
    density: 14.78,
    groupBlock: "actinoid",
    yearDiscovered: 1949,
    block: "f",
    cpkHexColor: "8A4FE3",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 98,
    symbol: "Cf",
    name: "Californium",
    atomicMass: [251],
    electronicConfiguration: "[Rn] 5f10 7s2",
    electronegativity: 1.3,
    atomicRadius: "unknown",
    ionRadius: "95 (+3)",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 608,
    electronAffinity: "unknown",
    oxidationStates: "2, 3, 4",
    standardState: "solid",
    bondingType: "metallic",
    meltingPoint: 1173,
    boilingPoint: "unknown",
    density: 15.1,
    groupBlock: "actinoid",
    yearDiscovered: 1950,
    block: "f",
    cpkHexColor: "A136D4",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 99,
    symbol: "Es",
    name: "Einsteinium",
    atomicMass: [252],
    electronicConfiguration: "[Rn] 5f11 7s2",
    electronegativity: 1.3,
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 619,
    electronAffinity: "unknown",
    oxidationStates: "2, 3",
    standardState: "solid",
    bondingType: "unknown",
    meltingPoint: 1133,
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "actinoid",
    yearDiscovered: 1952,
    block: "f",
    cpkHexColor: "B31FD4",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 100,
    symbol: "Fm",
    name: "Fermium",
    atomicMass: [257],
    electronicConfiguration: "[Rn] 5f12 7s2",
    electronegativity: 1.3,
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 627,
    electronAffinity: "unknown",
    oxidationStates: "2, 3",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: 1800,
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "actinoid",
    yearDiscovered: 1952,
    block: "f",
    cpkHexColor: "B31FBA",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 101,
    symbol: "Md",
    name: "Mendelevium",
    atomicMass: [258],
    electronicConfiguration: "[Rn] 5f13 7s2",
    electronegativity: 1.3,
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 635,
    electronAffinity: "unknown",
    oxidationStates: "2, 3",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: 1100,
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "actinoid",
    yearDiscovered: 1955,
    block: "f",
    cpkHexColor: "B30DA6",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 102,
    symbol: "No",
    name: "Nobelium",
    atomicMass: [259],
    electronicConfiguration: "[Rn] 5f14 7s2",
    electronegativity: 1.3,
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: 642,
    electronAffinity: "unknown",
    oxidationStates: "2, 3",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: 1100,
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "actinoid",
    yearDiscovered: 1957,
    block: "f",
    cpkHexColor: "BD0D87",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 103,
    symbol: "Lr",
    name: "Lawrencium",
    atomicMass: [262],
    electronicConfiguration: "[Rn] 5f14 7s2 7p1",
    electronegativity: 1.3,
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: 3,
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: 1900,
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "transition metal",
    yearDiscovered: 1961,
    block: "d",
    cpkHexColor: "C70066",
    period: 7,
    group: 3,
  },
  {
    atomicNumber: 104,
    symbol: "Rf",
    name: "Rutherfordium",
    atomicMass: [267],
    electronicConfiguration: "[Rn] 5f14 6d2 7s2",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: 4,
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "transition metal",
    yearDiscovered: 1969,
    block: "d",
    cpkHexColor: "CC0059",
    period: 7,
    group: 4,
  },
  {
    atomicNumber: 105,
    symbol: "Db",
    name: "Dubnium",
    atomicMass: [268],
    electronicConfiguration: "[Rn] 5f14 6d3 7s2",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: "unknown",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "transition metal",
    yearDiscovered: 1967,
    block: "d",
    cpkHexColor: "D1004F",
    period: 7,
    group: 5,
  },
  {
    atomicNumber: 106,
    symbol: "Sg",
    name: "Seaborgium",
    atomicMass: [271],
    electronicConfiguration: "[Rn] 5f14 6d4 7s2",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: "unknown",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "transition metal",
    yearDiscovered: 1974,
    block: "d",
    cpkHexColor: "D90045",
    period: 7,
    group: 6,
  },
  {
    atomicNumber: 107,
    symbol: "Bh",
    name: "Bohrium",
    atomicMass: [272],
    electronicConfiguration: "[Rn] 5f14 6d5 7s2",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: "unknown",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "transition metal",
    yearDiscovered: 1976,
    block: "d",
    cpkHexColor: "E00038",
    period: 7,
    group: 7,
  },
  {
    atomicNumber: 108,
    symbol: "Hs",
    name: "Hassium",
    atomicMass: [270],
    electronicConfiguration: "[Rn] 5f14 6d6 7s2",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: "unknown",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "transition metal",
    yearDiscovered: 1984,
    block: "d",
    cpkHexColor: "E6002E",
    period: 7,
    group: 8,
  },
  {
    atomicNumber: 109,
    symbol: "Mt",
    name: "Meitnerium",
    atomicMass: [276],
    electronicConfiguration: "[Rn] 5f14 6d7 7s2",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: "unknown",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "transition metal",
    yearDiscovered: 1982,
    block: "d",
    cpkHexColor: "EB0026",
    period: 7,
    group: 9,
  },
  {
    atomicNumber: 110,
    symbol: "Ds",
    name: "Darmstadtium",
    atomicMass: [281],
    electronicConfiguration: "[Rn] 5f14 6d9 7s1",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: "unknown",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "transition metal",
    yearDiscovered: 1994,
    block: "d",
    cpkHexColor: "unknown",
    period: 7,
    group: 10,
  },
  {
    atomicNumber: 111,
    symbol: "Rg",
    name: "Roentgenium",
    atomicMass: [280],
    electronicConfiguration: "[Rn] 5f14 6d10 7s1",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: "unknown",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "transition metal",
    yearDiscovered: 1994,
    block: "d",
    cpkHexColor: "unknown",
    period: 7,
    group: 11,
  },
  {
    atomicNumber: 112,
    symbol: "Cn",
    name: "Copernicium",
    atomicMass: [285],
    electronicConfiguration: "[Rn] 5f14 6d10 7s2",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: "unknown",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "transition metal",
    yearDiscovered: 1996,
    block: "d",
    cpkHexColor: "unknown",
    period: 7,
    group: 12,
  },
  {
    atomicNumber: 113,
    symbol: "Nh",
    name: "Nihonium",
    atomicMass: [284],
    electronicConfiguration: "[Rn] 5f14 6d10 7s2 7p1",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: "unknown",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "post-transition metal",
    yearDiscovered: 2003,
    block: "p",
    cpkHexColor: "unknown",
    period: 7,
    group: 13,
  },
  {
    atomicNumber: 114,
    symbol: "Fl",
    name: "Flerovium",
    atomicMass: [289],
    electronicConfiguration: "[Rn] 5f14 6d10 7s2 7p2",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: "unknown",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "post-transition metal",
    yearDiscovered: 1998,
    block: "p",
    cpkHexColor: "unknown",
    period: 7,
    group: 14,
  },
  {
    atomicNumber: 115,
    symbol: "Mc",
    name: "Moscovium",
    atomicMass: [288],
    electronicConfiguration: "[Rn] 5f14 6d10 7s2 7p3",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: "unknown",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "post-transition metal",
    yearDiscovered: 2003,
    block: "p",
    cpkHexColor: "unknown",
    period: 7,
    group: 15,
  },
  {
    atomicNumber: 116,
    symbol: "Lv",
    name: "Livermorium",
    atomicMass: [293],
    electronicConfiguration: "[Rn] 5f14 6d10 7s2 7p4",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: "unknown",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "post-transition metal",
    yearDiscovered: 2000,
    block: "p",
    cpkHexColor: "unknown",
    period: 7,
    group: 16,
  },
  {
    atomicNumber: 117,
    symbol: "Ts",
    name: "Tennessine",
    atomicMass: [294],
    electronicConfiguration: "[Rn] 5f14 6d10 7s2 7p5",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: "unknown",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "post-transition metal",
    yearDiscovered: 2010,
    block: "p",
    cpkHexColor: "unknown",
    period: 7,
    group: 17,
  },
  {
    atomicNumber: 118,
    symbol: "Og",
    name: "Oganesson",
    atomicMass: [294],
    electronicConfiguration: "[Rn] 5f14 6d10 7s2 7p6",
    electronegativity: "unknown",
    atomicRadius: "unknown",
    ionRadius: "unknown",
    vanDerWaalsRadius: "unknown",
    ionizationEnergy: "unknown",
    electronAffinity: "unknown",
    oxidationStates: "unknown",
    standardState: "unknown",
    bondingType: "unknown",
    meltingPoint: "unknown",
    boilingPoint: "unknown",
    density: "unknown",
    groupBlock: "noble gas",
    yearDiscovered: 2002,
    block: "p",
    cpkHexColor: "unknown",
    period: 7,
    group: 18,
  },
];

function App() {
  const groupTableStyles = {
    width: "12.5%",
    justifyContent: "center",
    textAlign: "center",
  };
  const SingleElementTile = ({ number, elementType }) => {
    return (
      <HoverCard
        width={320}
        shadow="md"
        withArrow
        openDelay={200}
        closeDelay={100}
      >
        <HoverCardTarget>
          <Card
            shadow="sm"
            radius="md"
            withBorder
            className={`bgtile elementTile ${elementType}`}
            style={{ display: "block" }}
          >
            <Card.Section
              style={{
                height: "100%",
                width: "100%",
              }}
            >
              {elementData[number].symbol}
            </Card.Section>
          </Card>
        </HoverCardTarget>
        <HoverCardDropdown>
          <Group
            style={{
              color: "white",
              backgroundColor: "grey",
              width: "300px",
              zIndex: "10000000",
              marginLeft: "2vw",
              marginBottom: "100%",
              marginTop: "0px",
            }}
          >
            <h1>{elementData[number].name}</h1>
            <Stack gap={5}>
              <Text size="sm" fw={700} style={{ lineHeight: 1 }}>
                Atomic Number: {elementData[number].atomicNumber}
              </Text>
              <Text size="sm" fw={700} style={{ lineHeight: 1 }}>
                Atomic Mass: {elementData[number].atomicMass}
              </Text>
              <Text size="sm" fw={700} style={{ lineHeight: 1 }}>
                Period: {elementData[number].period}
              </Text>
              <Text size="sm" fw={700} style={{ lineHeight: 1 }}>
                Group: {elementData[number].group}
              </Text>
            </Stack>
          </Group>
        </HoverCardDropdown>
      </HoverCard>
    );
  };
  function Table() {
    return (
      <Container className="bgtile" id="tablebg">
        <ScrollArea scrollbars="x" w="100%" type="hover">
          <Container id="tableTop">
            <div className="tableGroup" id="group1" style={groupTableStyles}>
              <SingleElementTile number={1} elementType="rnm" />
              <SingleElementTile number={3} elementType="am" />
              <SingleElementTile number={11} elementType="am" />
              <SingleElementTile number={19} elementType="am" />
              <SingleElementTile number={37} elementType="am" />
              <SingleElementTile number={55} elementType="am" />
              <SingleElementTile number={87} elementType="am" />
            </div>
            <div className="tableGroup" id="group2" style={groupTableStyles}>
              <SingleElementTile number={4} elementType="aem" />
              <SingleElementTile number={12} elementType="aem" />
              <SingleElementTile number={20} elementType="aem" />
              <SingleElementTile number={38} elementType="aem" />
              <SingleElementTile number={56} elementType="aem" />
            </div>
            <div className="tableGroup" id="group3" style={groupTableStyles}>
              <SingleElementTile number={21} elementType="tm" />
              <SingleElementTile number={39} elementType="tm" />
              <SingleElementTile number={57} elementType="tm" />
              <SingleElementTile number={89} elementType="tm" />
            </div>
            <div className="tableGroup" id="group4" style={groupTableStyles}>
              <SingleElementTile number={22} elementType="tm" />
              <SingleElementTile number={40} elementType="tm" />
              <SingleElementTile number={72} elementType="tm" />
              <SingleElementTile number={104} elementType="tm" />
            </div>
            <div className="tableGroup" id="group5" style={groupTableStyles}>
              <SingleElementTile number={23} elementType="tm" />
              <SingleElementTile number={41} elementType="tm" />
              <SingleElementTile number={73} elementType="tm" />
              <SingleElementTile number={105} elementType="tm" />
            </div>
            <div className="tableGroup" id="group6" style={groupTableStyles}>
              <SingleElementTile number={24} elementType="tm" />
              <SingleElementTile number={42} elementType="tm" />
              <SingleElementTile number={74} elementType="tm" />
              <SingleElementTile number={106} elementType="tm" />
            </div>
            <div className="tableGroup" id="group7" style={groupTableStyles}>
              <SingleElementTile number={25} elementType="tm" />
              <SingleElementTile number={43} elementType="tm" />
              <SingleElementTile number={75} elementType="tm" />
              <SingleElementTile number={107} elementType="tm" />
            </div>
            <div className="tableGroup" id="group8" style={groupTableStyles}>
              <SingleElementTile number={26} elementType="tm" />
              <SingleElementTile number={44} elementType="tm" />
              <SingleElementTile number={76} elementType="tm" />
              <SingleElementTile number={108} elementType="tm" />
            </div>
            <div className="tableGroup" id="group9" style={groupTableStyles}>
              <SingleElementTile number={27} elementType="tm" />
              <SingleElementTile number={45} elementType="tm" />
              <SingleElementTile number={77} elementType="tm" />
              <SingleElementTile number={109} elementType="idek" />
            </div>
            <div className="tableGroup" id="group10" style={groupTableStyles}>
              <SingleElementTile number={28} elementType="tm" />
              <SingleElementTile number={46} elementType="tm" />
              <SingleElementTile number={78} elementType="tm" />
              <SingleElementTile number={110} elementType="idek" />
            </div>
            <div className="tableGroup" id="group11" style={groupTableStyles}>
              <SingleElementTile number={29} elementType="tm" />
              <SingleElementTile number={47} elementType="tm" />
              <SingleElementTile number={79} elementType="tm" />
              <SingleElementTile number={111} elementType="idek" />
            </div>
            <div className="tableGroup" id="group12" style={groupTableStyles}>
              <SingleElementTile number={30} elementType="tm" />
              <SingleElementTile number={48} elementType="tm" />
              <SingleElementTile number={80} elementType="tm" />
              <SingleElementTile number={112} elementType="idek" />
            </div>
            <div className="tableGroup" id="group13" style={groupTableStyles}>
              <SingleElementTile number={5} elementType="m" />
              <SingleElementTile number={13} elementType="ptm" />
              <SingleElementTile number={31} elementType="ptm" />
              <SingleElementTile number={49} elementType="ptm" />
              <SingleElementTile number={81} elementType="ptm" />
              <SingleElementTile number={113} elementType="idek" />
            </div>
            <div className="tableGroup" id="group14" style={groupTableStyles}>
              <SingleElementTile number={6} elementType="rnm" />
              <SingleElementTile number={14} elementType="m" />
              <SingleElementTile number={32} elementType="m" />
              <SingleElementTile number={50} elementType="ptm" />
              <SingleElementTile number={82} elementType="ptm" />
              <SingleElementTile number={114} elementType="idek" />
            </div>
            <div className="tableGroup" id="group15" style={groupTableStyles}>
              <SingleElementTile number={7} elementType="rnm" />
              <SingleElementTile number={15} elementType="rnm" />
              <SingleElementTile number={33} elementType="m" />
              <SingleElementTile number={51} elementType="m" />
              <SingleElementTile number={83} elementType="ptm" />
              <SingleElementTile number={115} elementType="idek" />
            </div>
            <div className="tableGroup" id="group16" style={groupTableStyles}>
              <SingleElementTile number={8} elementType="rnm" />
              <SingleElementTile number={16} elementType="rnm" />
              <SingleElementTile number={34} elementType="rnm" />
              <SingleElementTile number={52} elementType="m" />
              <SingleElementTile number={84} elementType="ptm" />
              <SingleElementTile number={116} elementType="idek" />
            </div>
            <div className="tableGroup" id="group17" style={groupTableStyles}>
              <SingleElementTile number={9} elementType="rnm" />
              <SingleElementTile number={17} elementType="rnm" />
              <SingleElementTile number={35} elementType="rnm" />
              <SingleElementTile number={53} elementType="rnm" />
              <SingleElementTile number={85} elementType="ptm" />
              <SingleElementTile number={117} elementType="idek" />
            </div>
            <div className="tableGroup" id="group18" style={groupTableStyles}>
              <SingleElementTile number={2} elementType="ng" />
              <SingleElementTile number={10} elementType="ng" />
              <SingleElementTile number={18} elementType="ng" />
              <SingleElementTile number={36} elementType="ng" />
              <SingleElementTile number={54} elementType="ng" />
              <SingleElementTile number={86} elementType="ng" />
              <SingleElementTile number={118} elementType="idek" />
            </div>
          </Container>
          <br />
          <br />
          <Container
            id="tableBottom"
            style={{
              justifyContent: "center",
              margin: "auto",
              width: "100%",
              display: "block",
            }}
          >
            <Container
              id="1"
              style={{
                display: "flex",
                width: "100%",
                margin: "auto",
                textAlign: "center",
                justifyContent: "center",
              }}
            >
              <SingleElementTile number={58} elementType="l" />
              <SingleElementTile number={59} elementType="l" />
              <SingleElementTile number={60} elementType="l" />
              <SingleElementTile number={61} elementType="l" />
              <SingleElementTile number={62} elementType="l" />
              <SingleElementTile number={63} elementType="l" />
              <SingleElementTile number={64} elementType="l" />
              <SingleElementTile number={65} elementType="l" />
              <SingleElementTile number={66} elementType="l" />
              <SingleElementTile number={67} elementType="l" />
              <SingleElementTile number={68} elementType="l" />
              <SingleElementTile number={69} elementType="l" />
              <SingleElementTile number={70} elementType="l" />
              <SingleElementTile number={71} elementType="l" />
            </Container>
            <Container
              id="2"
              style={{
                display: "flex",
                width: "100%",
                margin: "auto",
                textAlign: "center",
                justifyContent: "center",
              }}
            >
              <SingleElementTile number={90} elementType="a" />
              <SingleElementTile number={91} elementType="a" />
              <SingleElementTile number={92} elementType="a" />
              <SingleElementTile number={93} elementType="a" />
              <SingleElementTile number={94} elementType="a" />
              <SingleElementTile number={95} elementType="a" />
              <SingleElementTile number={96} elementType="a" />
              <SingleElementTile number={97} elementType="a" />
              <SingleElementTile number={98} elementType="a" />
              <SingleElementTile number={99} elementType="a" />
              <SingleElementTile number={100} elementType="a" />
              <SingleElementTile number={101} elementType="a" />
              <SingleElementTile number={102} elementType="a" />
              <SingleElementTile number={103} elementType="a" />
            </Container>
          </Container>
        </ScrollArea>
      </Container>
    );
  }
  return (
    <MantineProvider>
      <div className="App" id="mainDiv">
        <video id="background-video" autoPlay loop muted>
          <source src={background} type="video/mp4" />
        </video>
        <h1 className="bgtile" id="pageHeader">
          {" "}
          The Elements
        </h1>
        <br />
        <br />
        <Table />
        <br />
        <br />;
      </div>
    </MantineProvider>
  );
}

export default App;
